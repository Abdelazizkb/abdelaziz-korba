    ...
    return (
        <div className="relative flex flex-col items-center w-auto md:w-custom overflow-hidden">
            <div className="flex justify-between items-center">
                <img className="cursor-pointer" src={back} onClick={()=>{handleClick(-1)}} />
                {(show==1)&&<Development/>}
                {(show==2)&&<Design/>}
                {(show==3)&&<Api/>}
                 
                <img className="cursor-pointer" src={next} onClick={()=>{handleClick(+1)}}/>
            </div>
            <div className="mt-8 md:mt-20 py-1 flex">
                <img className={((show==1)&&"animate-spin-rectangle")} src={rectangle} />
                <img className={`mx-2 ${(show==2)&&" animate-pulse"}`} src={ellipse} />
                <img className={((show==3)&&"animate-bounce")} src={polygon} />
            </div>
        </div>
    )
}
